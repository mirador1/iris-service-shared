...
- **Operational hygiene scripts** : default-branch verification, runner
  ...
  applicable.
- **SLO-as-code rules + Grafana dashboards** : 4 dashboards (SLO Overview +
  breakdown by endpoint + latency heatmap + Apdex) auto-imported via
  dashboard provisioning. PrometheusRules generated by Sloth from the
  consuming repos' `slo.yaml`.
...
| `infra/observability/grafana/dashboards-lgtm/` | 4 Grafana dashboards (SLO + breakdown + heatmap + Apdex) | LGTM provisioning |
| `infra/observability/` | OTel Collector + Prometheus config | java + python |
| `deploy/kubernetes/observability-prom/mirador-{slo,py-slo}.yaml` | Sloth-generated PrometheusRules | kube-prometheus-stack operator |
| `deploy/kubernetes/` | K8s manifests (Argo CD, ESO, Argo Rollouts, network policies) | clusters |
